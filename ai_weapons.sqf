// Set up Spetsnaz AI weapons so that VSS guys are left untouched and everyone else gets an AK-74SU
if (!isServer) exitWith {};

waitUntil {scriptDone f_script_setLocalVars};
{
	if (_x isKindOf "RUS_Soldier_Base" && !(_x hasWeapon "VSS_Vintorez")) then {
		_unit = _x;
